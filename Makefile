...
.DEFAULT_GOAL := help
.PHONY: crud blog login help
project := crud login blog

all: $(project) ## Generate Pbs and build

crud: $@ ## Generate Pbs and build for crud
login: $@ ## Generate Pbs and build for login
...
clean: clean_crud clean_login clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_crud: ## Clean generated files for crud
	${RM_F_CMD} crud/${PROTO_DIR}/*.pb.go

clean_login: ## Clean generated files for login
	${RM_F_CMD} login/${PROTO_DIR}/*.pb.go
